...
const App = () => {
  ...
  const [isLoggedStateCalculated, setIsLoggedStateCalculated] =
    useState<boolean>(false)

  // Redux login state
  useEffect(() => {
    const rawToken = localStorage.getItem('MW_MP_TOKEN_ADMIN')

    // Если токена нет вообще - пользователь не залогинен
    if (!rawToken) {
    ...
    try {
      ...
      const isLogged =
        !!tokenData &&
        tokenData.accessCreated + tokenData.accessExpired * SECOND_IN_MS >
          new Date().valueOf()
      ...
      // Если токен невалидный JSON - удаляем и считаем незалогиненным
